> **Server variant:** generation runs inside yue2-server. `scripts/run_yue2.py` submits the request
> fields below as a job and downloads the same artifacts; the `YuE2Pipeline` examples describe what the
> server executes and are not run by this skill.

# Generation, covers and reusable stages

Use the `yue2-infer` 0.1.6 interface described here. Installation and model snapshots are the server operator's job (`setup.sh` in the yue2-server repository); the helpers record the server's model revisions and weight identities with each run. Validate a new installation with fresh outputs before treating it as a reproduced experiment.
...
For an audio-based cover, transcribe the source recording with SheetSage2, review its melody and timing, remove the chord annotations, then use the same call. Obtain or check the lyrics separately. The transcription chain (SheetSage2, which loads its MERT-v2-FullSong encoder itself) runs on the server through `scripts/transcribe.py`. Source-separation, transcription, lyric recognition and score-conditioned generation are distinct operations; YuE2 has no direct audio-upload argument in this interface. Its VAE encoder is not a substitute for melody transcription.
